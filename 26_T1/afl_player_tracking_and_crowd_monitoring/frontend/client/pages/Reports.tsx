...
type ReportItem = (typeof availableReports)[0] & {
  isLive?: boolean;
  rawData?: any;
};
...
const buildLiveReportText = (data: any) => {
...
Highest risk zone: ${crowd.highest_risk_zone}`
  ...
  return `AFL VIDEO ANALYSIS REPORT (Live — from your uploaded video)
...
export default function Reports() {
  ...
  const [liveReport, setLiveReport] = useState<ReportItem | null>(null);
    ...
    fetch(`${BACKEND_URL}/api/analytics/latest`, {
      ...
      .then((data) => {
        const created = data.created_at
          ...
          : new Date();
        const teamCounts = data.player?.tracking?.team_detection_counts;
        const teams =
          teamCounts && Object.keys(teamCounts).length > 0
            ? Object.keys(teamCounts).join(" vs ")
            : "Your uploaded video";

        setLiveReport({
          id: `live-${data.job_id}`,
          name: `Your Video Analysis (Job ${String(data.job_id).slice(0, 8)})`,
          type: "Full Analysis Report",
          category: data.crowd ? "Crowd Monitoring" : "Player Analysis",
  ...
  const allReports: ReportItem[] = liveReport
    ? [liveReport, ...availableReports]
    : availableReports;
...
This report was generated by AFL Analytics Platform.`;
  ...
  const downloadReport = (report: ReportItem) => {
    try {
      const content = report.isLive && report.rawData
        ? buildLiveReportText(report.rawData)
        : `AFL Analytics Report: ${report.name}
